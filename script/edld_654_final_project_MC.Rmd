...
teacher_clean <- teacher_clean %>% 
  mutate(
    assign_pro_bi = ifelse(IT3G17J == 1,0,1)
  ) %>% 
...
```{r}
missing_ <- ff_glimpse(teacher_clean)$Categorical
...
```{r}
group_0 <- which(teacher_te$assign_pro_bi=="Never")
group_1 <- which(teacher_te$assign_pro_bi=="AnyUse")

prob_0 <- jitter(predicted_te$AnyUse[group_0], amount=1e-4)
prob_1 <- jitter(predicted_te$AnyUse[group_1], amount=1e-4)

plot(density(prob_0, adjust=2), col = "lightblue", xlab='Model Prediction', main = "")
lines(density(prob_1, adjust=2),col = "orange",lty=2)
legend("topright", c("Never","AnyUse"), col = c("lightblue","orange"), lty=c(1,2), bty='n')
...
cut.obj <- cutpointr(x = predicted_te$AnyUse,
...
pred_class <- ifelse(predicted_te$AnyUse>.5,1,0)
...
plot(density(predicted_ridge_te[group_0,]$AnyUse, adjust = 1.5),col = "lightblue",xlab = 'Model Prediction', main = "Logistic Model_ridge penalty")
points(density(predicted_ridge_te[group_1,]$AnyUse, adjust = 1.5),col = "orange",lty=2,type='l')
...
```{r}
cut.obj_ridge <- cutpointr(x = predicted_ridge_te$AnyUse,
                           class = teacher_te$assign_pro_bi)
...
```{r}
pred_ridge_class <- ifelse(predicted_ridge_te$AnyUse>.5,1,0)
...
```{r}
plot(density(predicted_lasso_te[group_0,]$AnyUse,adjust=1.5), col="lightblue",xlab = '', main = 'Logistic Model_lasso penalty')
points(density(predicted_lasso_te[group_1,]$AnyUse,adjust=1.5),col="orange",lty=2,type='l')
legend("topright", c("Never","AnyUse"), col = c("lightblue","orange"), lty=c(1,2), bty='n')

```

```{r}
cut.obj_lasso <- cutpointr(x = predicted_lasso_te$AnyUse,
                           class = teacher_te$assign_pro_bi)
AUC_lasso <- auc(cut.obj_lasso)
```

```{r}
pred_lasso_class <- ifelse(predicted_lasso_te$AnyUse>.5,1,0)
