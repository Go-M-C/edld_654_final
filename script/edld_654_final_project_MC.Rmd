...
cut.obj <- cutpointr(data = teacher_te,
                     x = prob_never,
                     class = assign_pro_bi,
                     pos_class = "Never",
...
1.4.1 Train the logistic regression model
...
1.4.2 Logistic Regression Performance Evaluation
...
plot(density(predicted_ridge_te[group_0,]$AnyUse, adjust = 1), xlab = '', main = "Logistic Model_ridge penalty")

points(density(predicted_ridge_te[group_1,]$AnyUse,
               adjust = 1),lty=2,type='l')

legend(x=0.8, y=2.75, c("AnyUse","Never"))
```
