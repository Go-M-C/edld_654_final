...
```{r}
# Checking outcome variable IT3G17J: proportion of lessons in your
# reference class do students do Write computer programs,games, or apps
# (e.g., using [Python, LUA,Javascript,Scratch])

attributes((teacher$IT3G17J)$labels)
# Convert to factor
require(haven)
teacher_clean$IT3G17J_f <- haven::as_factor(teacher_clean$IT3G17J)

# Remove label and the missing value
teacher_clean$IT3G17J_num <- as.numeric(teacher_clean$IT3G17J)
teacher_clean$IT3G17J_num[teacher_clean$IT3G17J_num %in% c(8,9)] <- NA

teacher_clean$IT3G17J_ord <- factor(
  teacher_clean$IT3G17J_num,
  levels = 1:5,
  labels = c("Never",
             "One quarter or less",
             "One half or less",
...
table(teacher_clean$IT3G17J_ord, useNA = "ifany")

# Visualize the distribution

ggplot(teacher_clean, aes(IT3G17J_ord)) +
...
-   I collapsed the outcome variable into a binary indicator that reflecting whether the teacher ever engages students in computer programming activities. The original outcome is highly skewed toward "Never", and the categories above "Never" are sparse. It would be risky for me to use ordinal modeling. 
