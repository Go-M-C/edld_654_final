...
1.3. Logistic Regression with No Regularization:
...
```{r}
LL_mod <- caret_mod$results$logLoss
```

1.3.1 Logistic Regression Performance Evaluation

```{r}
predicted_te <- predict(caret_mod, teacher_te, type = 'prob')
dim(predicted_te)
```
...
```{r}
# library(pROC)
# roc_obj <- roc(teacher_te$assign_pro_bi, predicted_te$Never, levels=c("AnyUse","Never"), direction="<")
# auc(roc_obj)
# plot(roc_obj)

# This chunk can directly visualize specificity and sensitivity
```
...
teacher_te$prob_never <- predicted_te$Never

cut.obj_mod <- cutpointr(data = teacher_te,
                     x = prob_never,
                     class = assign_pro_bi,
                     pos_class = "Never",
...
confusion_mod <- table(teacher_te$assign_pro_bi,pred_class)
...
1.4 Train the logistic regression model

```{r}
Sys.time()

caret_logistic_ridge <- train(blueprint,
                              data      = teacher_tr,
                              method    = "glmnet",
                              family    = "binomial",
                              metric    = "logLoss",
